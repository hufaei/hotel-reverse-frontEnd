<template>
  ...
  import { sendEmail, verifyCode, verifyEmail} from '@/api/modules/system/verify';
  import { resetPassword } from '@/api/modules/system/user';
  ...
  const handleCheckCode = async (data?: any) => {
    try {
      const result = await verifyCode({Remail: Remail.value, code: data.code});
      if (result) {
        return true;
      } else {
  ...
  // 重置密码逻辑（添加延迟跳转）
  const handleResetPassword = async (data:IUser.PasswordForm) => {
    try {
